...
//LK
ElfLinker::Symbol *ElfLinker::LKfindSymbol(const char *name) const {
...
ElfLinker::Symbol *ElfLinker::addSymbol(const char *name, const char *section,
                                        upx_uint64_t offset)
{
    //printf("addSymbol: %s %s 0x%x\n", name, section, (unsigned int) offset);
...
int ElfLinker::addLoader(const char *sname)
{
    ...
    for (char *sect = begin; sect < end; )
    {
        ...
        {
            //printf("[LK] [LIST] %s\n", sect);
            //printf("[LK] [DEBUG] sect[0] is +: %s\n", sect);
            assert(tail);
            //printf("[LK] [DEBUG] sect[2]: %c\thex(sect[2]):%u \n", sect[2], hex(sect[2]));
            //printf("[LK] [DEBUG] tail->offset: %u\n", (unsigned int) tail->offset);
            //printf("[LK] [DEBUG] tail->size: %u\n", (unsigned int) tail->size);
            //printf("[LK] [DEBUG] sect[1]: %c\thex(sect[1]): %u\n", sect[1], hex(sect[1]));
...
{
    //printf("[LK] [DEBUG] alignWithByte -> len: %u b: %c", len, b);
...
{
    //printf("[LK] [DEBUG] alignCode: %u", len);
    assert((len & 1) == 0);
